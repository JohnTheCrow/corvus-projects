...
if [[ ! -f proc/sys/kernel/tainted ]]; then
	echo Can\'t find proc/sys/kernel/tainted. Are you in the root of a sosreport?
	exit 1;
fi
...
echo '      2048 - The kernel is working around a severe bug in the platform'
echo '             firmware (BIOS or similar)' 
echo ' 268435456 - Unsupported hardware. Refer to "UNSUPPORTED HARDWARE'
echo '             DEVICE:" kernel log entry for details.'
echo ' 536870912 - Technology Preview code was loaded; cf.  Refer to'
echo '             "TECH PREVIEW:" kernel log entry for details.'
